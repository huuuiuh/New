...
## Phase 2 — Planning & production pipeline (Checkpoint 5 slice delivered in this change: B-2-5 core loop, B-2-8 commit path, B-2-10 resume, B-2-17 accepted-only export; remainder stays open)
...
| B-2-5 | P0 | ✅ ChapterProduction workflow: preflight (previous-chapter gate before spend, ADR-0046), scene plan (register pre-resolution), scene writer loop with **output-language gate**, assembler, deterministic checks | FR-4.1–4.4, FR-4.10 | happy path w/ mock; Korean-output mock rejected | 6d |
...
| B-2-10 | P0 | ✅ Batch workflow; pause/cancel/resume; leases; stale-canon re-validation — resume slice: deterministic `workflowId`, `runStep`/`job_steps` idempotent resume, `failAfterStep` proof (T19) | FR-4.7, FR-7.12/7.13, NFR-B | chaos cases | 4d |
...
| B-2-17 | P1 | ✅ Export accepted-only workflow (`exportAccepted`: accepted manuscripts only; quarantine/working never export) — TXT/DOCX profiles remain follow-up | FR-10.1 | typography check | 3d |
