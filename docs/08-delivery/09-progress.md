...
| 5 | Chapter-production vertical slice (deterministic Postgres-checkpointed core loop: intake → spec → bible → arc → locked contract → scene plan → draft → checks → bounded revision → approval-lock → extraction → verification → atomic commit → L1 summary + accepted-only index + dependency edges; ch.1 → ch.2 carries summary/tail/hook/deltas; `exportAccepted`; migration 0004 `jobs.workflow_id`/idempotency/`pins` + `workflow_artifacts`; Replay `activity:<id>` binding; ADR-0046) | `hoplite/kos-969b8d7e--checkpoint-05-completion` | done, awaiting review | `huuuiuh/New#1` |
...
| `DATABASE_URL=postgres://… CI=true pnpm check` | types-fresh → typecheck → lint → format:check → unit + Postgres integration tests → validator | **green**: 22 test files, 177 tests passed (Checkpoint 5 head `81e24a2`; 21 chapter-production integration; Postgres 16.14, Node 24.19.0, pnpm 10.26.0, Python 3.12.3) |
...
1. ~~`git checkout -b …--build-04-context-retrieval--build-05-chapter-vertical-slice` from the Checkpoint 4 head.~~
   Landed as `hoplite/kos-969b8d7e--checkpoint-05-completion` (`huuuiuh/New#1`, head `81e24a2`; repair of
   the WIP slice merged as PR #7 at `4ea8ecd`).
2. `packages/workflows` (Postgres-checkpointed idempotent steps, ADR-0044/ADR-0046): intake →
   ...
   proving it carries Chapter 1's summary/tail/hook/deltas → export. Done; 21/21 integration tests.
...
4. Follow-ups (not in this change): CLI `chapter:produce <project> <ch>` operator surface; per-project name
   thesaurus (B-1-13); Checkpoint 6 quality/long-form scope.
